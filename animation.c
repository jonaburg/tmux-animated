...
static void	animation_pane_draw(struct client *, struct animation *,
		    double);
static double	animation_ease_cubic_in_out(double);
    ...
    struct screen *src, int old_oy, int new_oy)
{
	...
	if (c->overlay_draw != NULL && c->animation == NULL)
		return;
	/*
	 * If a scroll animation is already in flight, replace old_oy with
	 * the current visual oy so the new animation continues from where
	 * the eye is and we avoid a snap-back.
	 */
